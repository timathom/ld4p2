...
Array.prototype.slice.call(document.documentElement.attributes).forEach(function(n) {
	if (n.nodeName.substr(0, 6) === "xmlns:") {
		...
		Fleur.XPathNSResolver_default.pf.push(n.prefix);
...
function XsltForms_model(subform, elt) {
	...
	Array.prototype.slice.call(elt.children).forEach(function(n) {
		if (n.localName.toLowerCase() === "script" && n.getAttribute("type") === "application/xml") {
			var doc = XsltForms_browser.createXMLDocument(n.textContent);
...
XsltForms_abstractAction.prototype.init = function(elt) {
	...
	this.element = elt;
	var mainaction = elt;
	while (mainaction && !mainaction.hasAttribute("ev-event")) {
...
XsltForms_select.prototype.initBody = function() {
	...
	if (this.full) {
		...
		select.innerHTML = Array.prototype.slice.call(cell.children).map(function(child) {
...
XsltForms_schema.getType = function(tname) {
	...
	return XsltForms_schema.getTypeNS(Fleur.XPathNSResolver_default.uri[Fleur.XPathNSResolver_default.pf.lastIndexOf(res[0])], res[1]);
};
