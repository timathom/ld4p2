...
<xsl:template match="xhtml:html | html" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:exslt="http://exslt.org/common" xmlns:xalan="http://xml.apache.org/xalan">
	...
	<html>
		...
		<xsl:if test="namespace::*">
			<xsl:for-each select="namespace::*[not(contains('..xml.', concat('.', name(), '.')))]">
...
<xsl:template match="xforms:*" priority="100" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xforms="http://www.w3.org/2002/xforms">
	...
	<xsl:choose>
		...
		<xsl:when test="contains('.bind.submission.itext.translation.text.group.repeat.input.output.textarea.secret.label.alert.hint.help.item.value.itemset.trigger.submit.range.upload.switch.case.select.select1.dialog.include.var.extension.', concat('.', local-name(), '.'))">
			<xsl:element name="{$elt_name}" namespace="{$elt_nsuri}">
				<xsl:if test="$custom_name != $elt_name">
					<xsl:attribute name="xforms-name"><xsl:value-of select="local-name()"/></xsl:attribute>
				</xsl:if>
		...
		<xsl:when test="contains('.setvalue.insert.delete.dispatch.property.targetid.name.action.load.toggle.send.setfocus.wrap.setselection.setindex.setnode.reset.refresh.rebuild.recalculate.revalidate.unload.hint.alert.help.value.item.itemset.copy.choices.filename.show.hide.method.resource.header.mediatype.message.control.', concat('.', local-name(), '.'))">
			<xsl:element name="{$elt_name}" namespace="{$elt_nsuri}">
				<xsl:if test="$custom_name != $elt_name">
					<xsl:attribute name="xforms-name"><xsl:value-of select="local-name()"/></xsl:attribute>
				</xsl:if>
